...
- `revision/_workspace/02_domain-researcher_design-big.md`
- `revision/_workspace/02_domain-researcher_design-big.json`
- `revision/_workspace/07_research-citation-auditor_design-big.md`
- `revision/_workspace/07_research-citation-auditor_design-big.json`

## Design Freeze Contract
- `create-plan.json` must assign every chapter a target word count, min/max range, status, retry policy, and required gates.
- `design-hashes.json` must hash the approved design and state files after `novel-config.md` is written.
- If any hashed design source changes after approval, create/polish/export must stop and rewrite must first produce `revision/_workspace/rewrite-impact-report.json`.
