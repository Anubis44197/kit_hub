...
Assert-PowerShellParses "scripts/revision_proposals.ps1"
Assert-PowerShellParses "scripts/apply_revision.ps1"

Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "/api/health" -Message "Studio bridge missing health endpoint."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "/api/provider-settings" -Message "Studio bridge missing provider settings endpoint."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "Protect-ProviderSecret" -Message "Studio bridge must protect API keys outside the repo."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "KITHUB_API_KEY" -Message "Studio bridge must load API keys into provider environment."
...
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "/api/revision-proposals" -Message "Studio bridge missing revision proposal endpoint."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "/api/apply-revision" -Message "Studio bridge missing approved revision apply endpoint."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "revision-proposals-approval.json" -Message "Studio bridge must support revision proposal approval."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "/api/version-history" -Message "Studio bridge missing version history endpoint."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "/api/restore-version" -Message "Studio bridge missing guarded version restore endpoint."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "New-ProjectVersionSnapshot" -Message "Studio bridge must create project version snapshots."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "revision/_versions" -Message "Studio bridge must store versions inside the project revision area."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern 'New-ProjectVersionSnapshot -ProjectRoot \$projectRoot -Reason' -Message "Version restore must snapshot the current state before restoring."
...
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "layout_profile" -Message "Studio bridge must persist the selected layout profile."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "book_template" -Message "Studio bridge must persist the selected full book template."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "runtime/layout-profile.json" -Message "Studio bridge must expose the runtime book template profile path."
Assert-Contains -RelativePath "runtime/layout-profile.schema.json" -Pattern '"book_template"' -Message "Layout profile schema must require the selected full book template."
Assert-Contains -RelativePath "runtime/layout-profile.schema.json" -Pattern '"page_numbering"' -Message "Layout profile schema must require page numbering rules."
Assert-Contains -RelativePath "scripts/local_phase.ps1" -Pattern "book_template = `"classicNovel`"" -Message "Local intake must seed a full book template, not only visual layout."
Assert-Contains -RelativePath "scripts/local_phase.ps1" -Pattern "page_numbering" -Message "Local intake must seed page numbering policy."
Assert-Contains -RelativePath "scripts/run_pipeline.ps1" -Pattern '"book_template","book_template_label"' -Message "Pipeline must validate full book template fields in layout plan."
Assert-Contains -RelativePath "scripts/run_pipeline.ps1" -Pattern "full book template field" -Message "Pipeline must block incomplete runtime book templates at approval time."
Assert-Contains -RelativePath "scripts/run_pipeline.ps1" -Pattern "page_setup missing" -Message "Pipeline must validate runtime page setup details."
Assert-Contains -RelativePath "scripts/run_pipeline.ps1" -Pattern "typography missing" -Message "Pipeline must validate runtime typography details."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "profile_status = `"USER_APPROVED`"" -Message "Studio bridge runtime book template must satisfy layout-profile schema status."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "trim_size = \`$trimSize" -Message "Studio bridge runtime page setup must include schema trim_size."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "body_style = `"KitHubBody`"" -Message "Studio bridge runtime typography must include body style."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "chapter_title_style = `"KitHubChapterTitle`"" -Message "Studio bridge runtime typography must include chapter title style."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "front_matter" -Message "Studio bridge must persist front matter requirements."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "cover" -Message "Studio bridge must persist cover package requirements."
Assert-Contains -RelativePath "scripts/studio_bridge.ps1" -Pattern "chapter_title_policy" -Message "Studio bridge must persist reader-facing chapter title policy."
...
Assert-Contains -RelativePath "index.html" -Pattern "api/provider-settings" -Message "Studio UI missing provider settings call."
Assert-Contains -RelativePath "index.html" -Pattern "apiProvider" -Message "Studio UI missing API provider selector."
Assert-Contains -RelativePath "index.html" -Pattern "apiKeyInput" -Message "Studio UI missing API key input."
Assert-Contains -RelativePath "index.html" -Pattern "copyIdeCommandBtn" -Message "Studio UI missing IDE setup command action."
...
Assert-Contains -RelativePath "index.html" -Pattern "api/revision-proposals" -Message "Studio UI missing revision proposal generation call."
Assert-Contains -RelativePath "index.html" -Pattern "api/apply-revision" -Message "Studio UI missing approved revision apply call."
Assert-Contains -RelativePath "index.html" -Pattern "revision-proposals-approval.json" -Message "Studio UI missing revision proposal approval support."
Assert-Contains -RelativePath "index.html" -Pattern "renderRevisionPanel" -Message "Studio UI missing revision card renderer."
Assert-Contains -RelativePath "index.html" -Pattern "renderVersionHistory" -Message "Studio UI missing version history renderer."
Assert-Contains -RelativePath "index.html" -Pattern "data-support-tab=`"versions`"" -Message "Studio UI missing compact version history access."
Assert-Contains -RelativePath "index.html" -Pattern "api/restore-version" -Message "Studio UI missing guarded version restore call."
Assert-Contains -RelativePath "index.html" -Pattern "data-version-restore" -Message "Studio UI missing active restore action."
...
Assert-Contains -RelativePath "index.html" -Pattern "runLogText" -Message "Studio UI must keep the last motor note internally."
Assert-NotContains -RelativePath "index.html" -Pattern "id=`"runLogOutput`"" -Message "Studio UI must not render technical run log output."
Assert-NotContains -RelativePath "index.html" -Pattern "id=`"copyRunLogBtn`"" -Message "Studio UI must not render technical run log copy button."
...
Assert-Contains -RelativePath "index.html" -Pattern "layout_profile" -Message "Studio UI must send selected layout profile to the bridge."
Assert-Contains -RelativePath "index.html" -Pattern "book_template" -Message "Studio UI must send selected book template to the bridge."
Assert-Contains -RelativePath "index.html" -Pattern "cover_brief_policy" -Message "Studio UI must send cover/package template requirements to the bridge."
Assert-Contains -RelativePath "index.html" -Pattern "chapter_start_policy" -Message "Studio UI must send chapter start template requirements to the bridge."
